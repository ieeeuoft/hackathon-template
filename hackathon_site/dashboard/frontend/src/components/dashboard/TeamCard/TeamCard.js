...
function ItemIcon(props) {
    if (props.icon === "location") {
        return <MapIcon onClick={handleClick}></MapIcon>;
    } else if (props.icon === "link") {
        return (
            <LinkIcon href="https://css-tricks.com/fixing-tables-long-strings/"></LinkIcon>
        );
    } else if (props.icon === "download") {
        return <GetAppIcon onClick={handleClick}></GetAppIcon>;
    }
    return;
}
