    ...
    def validate(self, data):
        ...
        for (hardware, requested_quantity) in requested_hardware.items():
            ...
            if (team_hardware_count + requested_quantity) > hardware.max_per_team:
                error_messages.append("Maximum number of items for Hardware {} is reached (limit of {} per team)".format(hardware.name, hardware.max_per_team))
        ...
        for (category, count) in category_counts.items():
            if count > category.max_per_team:
                error_messages.append("Maximum number of items for the Category {} is reached (limit of {} items per team)".format(category.name, category.max_per_team))
        if error_messages:
