...
def get_applications_to_date(hackathon:str, year:str):
    ...
    open_date = pd.to_datetime([REGISTRATION_OPEN_DATES[f"{hackathon}_{year}"]] * len(queried_data))
...
def create_json_data_file(json_data, hackathon, year, analytic_type):
    ...
    with open(f'{target_folder}/{hackathon}/{year}/{analytic_type}.json', 'w') as outfile:
