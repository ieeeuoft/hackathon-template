...
django.jQuery(document).ready(async () => {
    //-------------------------------- LOADING DATA -----------------------------------
    const timelineDataUnresolved = Object.entries(DATA_AVAILABLE).flatMap( ([hackathon, years]) => {
        return Object.keys(years).map(async (year) => {
            const data =  await django.jQuery.getJSON(`/static/registration/assets/${hackathon}/${year}/timeline.json`)
            return {
    ...
    for (const [hackathon, years] of Object.entries(DATA_AVAILABLE)) {
        ...
        for (const year in years) {
            const data =  await django.jQuery.getJSON(`/static/registration/assets/${hackathon}/${year}/categorized.json`)
            for (const [category, categoryData] of Object.entries(data)) {
                data[category].colors = Object.values(categoryData[`applicant_${category}`]).map(str => stringToColour(str))
            }
    ...
    const maxDayOfRegistration = timelineData.reduce((prev, curr) =>
        Math.max(prev.data.at(-1).day_of_registration, curr.data.at(-1).day_of_registration)
    )
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        title: () => ('Date'),
                        label: (context) => ` Day ${context.raw.day_of_registration}: ${context.raw.date}`
                    }
                }
            }
        }
    ...
    const getPieChartConfig = (hackathon, year, dataType) => ({
        type: 'pie',
        data: {
            labels: hackathon === "empty" ? [] : categorizedData[hackathon][year][dataType][`applicant_${dataType}`],
            datasets: [{
                label: hackathon === "empty" ? 'Placeholder' : `${hackathon} ${year}`,
                data: hackathon === "empty" ? [] : categorizedData[hackathon][year][dataType].num_applicants,
                backgroundColor: hackathon === "empty" ? [] : categorizedData[hackathon][year][dataType].colors,
                ...defaultSettings
            }]
        },
        options: {
            plugins: {
                ...
                title: {
                    display: true,
                    text: hackathon === "empty"
                        ? "Placeholder"
                        : capitalizeString(`${hackathon} ${year} applicant ${dataType.replace('_', ' ')}s`),
                    color: '#D7DBDD',
                    font: {
                        size: 18
                    }
    ...
    handleGraphChange = () => {
        ...
        if (chartType === "timeline") {
            ...
            if (hackathonYear === "") {
                alert("incorrect combination for data charts, please select a valid year.")
                return
            }
